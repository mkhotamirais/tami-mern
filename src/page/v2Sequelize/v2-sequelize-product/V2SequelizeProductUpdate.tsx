...
export default function V2SequelizeProductUpdate() {
  const { id } = useParams();
  const { singleData, getDataById, loadSingleData, errSingleData } = useV2Sequelize();
  ...
  const onSubmit = async (values: CreateProductForm) => {
    setPending(true);
    axios
      .patch(`${urlSequelize}/v2/product/${id}`, values)
      .then((res) => {
