...
import { Product, useV2Sequelize } from "@/hooks/useV2Sequelize";

export default function V2SequelizeProductDelDialog({ item }: { item: Product }) {
  const [pending, setPending] = useState(false);
  const { getData } = useV2Sequelize();
  const onDel = async () => {
    setPending(true);
    axios
      .delete(`${urlSequelize}/v2/product/${item.id}`)
      .then((res) => {
