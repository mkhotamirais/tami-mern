...
import { ProductSchema } from "./v1MysqlSchemas";
import { useV1Mysql } from "@/hooks/useV1Mysql";

type CreateProductForm = z.infer<typeof ProductSchema>;

export default function V1MysqlProductUpdate() {
  const { id } = useParams();
  const { singleData, getDataById, loadSingleData, errSingleData } = useV1Mysql();
  ...
  const onSubmit = async (values: CreateProductForm) => {
    setPending(true);
    axios
      .patch(`${urlMysql}/v1/product/${id}`, values)
      .then((res) => {
        toast.success(res.data.message);
        navigate("/v1-mysql/product");
      })
      .catch((err) => {
