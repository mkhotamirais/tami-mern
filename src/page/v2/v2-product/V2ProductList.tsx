...
import V2ProductDelDialog from "./V2ProductDelDialog";
import { V2Products } from "@/hooks/useV2";

export default function V2ProductList({ item }: { item: V2Products }) {
  return (
    <div className="group relative border p-2 rounded-lg overflow-hidden">
      ...
      <div className="scale-0 group-hover:scale-100 origin-right transition absolute flex gap-1 right-0 top-1/2 -translate-y-1/2 p-2">
        <Button size={"sm"} variant={"outline"} asChild>
          <Link to={`/v2-mongodb/product-update/${item._id}`}>
            <FaPenToSquare className="text-green-500" />
          </Link>
        </Button>
        <V2ProductDelDialog item={item} />
      </div>
    </div>
  );
}
