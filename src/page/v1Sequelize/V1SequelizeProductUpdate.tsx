...
export default function V1SequelizeProductUpdate() {
  const { id } = useParams();
  const { singleData, getDataById, loadSingleData, errSingleData } = useV1Sequelize();
  ...
  const onSubmit = async (values: CreateProductForm) => {
    setPending(true);
    axios
      .patch(`${urlSequelize}/v1/product/${id}`, values)
      .then((res) => {
